...
require 'zipfile'
require 'base64/urlsafe_b64encode'
...
def download_artifact_by_name(jobId, artifact_name, dest_file)
    ...
    puts "Fetching build job artifacts at #{url}"
...
def get_flet_server_job_ids()
    ...
    puts "Fetching build details at #{url}"
    req = urllib.request.Request(url)
    req.add_header("Content-type", "application/json")
...
def rehash(path, blocksize=1 << 20)
    ...
    h = hashlib.sha256()
    length = 0
    with open(path, "rb") |f| do 
        for block in read_chunks(f, size=blocksize) do
            length += len(block)
            h.update(block)
        end
    end
...
def rehash_record_lines(root_dir)
    lines = []
    for root, dirs, files in os.walk(root_dir, topdown=True) do
        for name in sorted(files) do
...
if len(whl_files) == 0
    puts "No .whl files found. Run 'pdm build' first."
    sys.exit(1)
end
...
puts "flet_server_jobId", build_jobs[fletd_job_name]

for name, package in packages.items() do
    puts "Building {name}..."

    puts "Unpacking original wheel file..."
    ...
    if flet_client_job
        ...
        if flet_client_filename.endswith(".zip")
            with zipfile.ZipFile(client_arch_path, "r")  |zip_arch| do
    ...
    # update WHEEL file
    for tag in package["wheel_tags"] do
        wheel_lines.append("Tag: #{tag}\n")
    end
    # save WHEEL
    with open(wheel_path, "w") |f| do
    ...
    record_path = str(
        unpacked_whl.joinpath("flet-#{package_version}.dist-info", "RECORD")
    )
    with open(record_path, "w") |f| do
        f.writelines(record_lines)
    end

    # zip
    suffix = package["file_suffix"]
    zip_filename = current_dir.joinpath("dist", "flet-#{package_version}-#{suffix}")
    shutil.make_archive(zip_filename, "zip", unpacked_whl)
    os.rename("#{zip_filename}.zip", "#{zip_filename}.whl")
